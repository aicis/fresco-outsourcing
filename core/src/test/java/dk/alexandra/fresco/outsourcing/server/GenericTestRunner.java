...
public class GenericTestRunner<ClientInputT, ClientOutputT, ServerOutputT> {
    ...
    public synchronized Map<Integer, Future<ClientInputT>> getFutureClientState() {
        return TestUtil.lazyEvaluationWithTimeout(() -> {
     ...
     * @throws InterruptedException
     */
    public Map<Integer, ClientOutputT> runOutputClients(Map<Integer, Integer> clientFacingPorts,
        OutputClientFunction<ClientOutputT> outputClientFunction) throws InterruptedException {
        List<Party> servers = new ArrayList(clientFacingPorts.size());
     ...
     * @throws InterruptedException
     */
    public Map<Integer, Future<ServerOutputT>> runServers(List<Integer> freePorts, BigInteger modulus) throws InterruptedException {
        if (freePorts.size() < 3 * getNumberOfServers()) {
            throw new RuntimeException("There must be at least 3 times the amount of servers open"
                + " ports");
        }
        ExecutorService es = Executors.newCachedThreadPool();
        // Server ID is contiguous and starts at 1
        List<Integer> serverIds = (List) IntStream.rangeClosed(1, this.getNumberOfServers()).boxed().collect(
            Collectors.toList());
        // Input client ID is contiguous and starts at 1
        List<Integer> inputIds = (List)IntStream.rangeClosed(1, this.getNumberOfInputClients()).boxed().collect(Collectors.toList());
        // Output client ID is contiguous and starts at the amount of input clients + 1
        List<Integer> outputIds = (List)IntStream.range(this.getNumberOfInputClients() + 1, this.getNumberOfInputClients() + 1 + this.getNumberOfOutputClients()).boxed().collect(Collectors.toList());
        Map<Integer, Future<SpdzWithIO>> spdzServers = new HashMap(this.getNumberOfServers());
        Map<Integer, Integer> internalPorts = SpdzSetup.getInternalPorts(freePorts, this.getNumberOfServers());
        ...
        while(servers.hasNext()) {
            ...
            Future<SpdzWithIO> spdzServer = es.submit(() -> {
                return new SpdzWithIO(serverId, SpdzSetup.getClientFacingPorts(freePorts, this.getNumberOfServers()),
                    internalPorts, SpdzSetup.getApplicationPorts(freePorts, this.getNumberOfServers()),

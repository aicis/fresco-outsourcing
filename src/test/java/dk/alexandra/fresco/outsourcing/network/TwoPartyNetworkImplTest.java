...
public class TwoPartyNetworkImplTest {
  ...
  public void setUp() throws Exception {
    ...
    Future<CloseableNetwork> n1 =
        es.submit(() -> new AsyncNetwork(conf.get(Parties.SERVER.id())));
    Future<CloseableNetwork> n2 =
        es.submit(() -> new AsyncNetwork(conf.get(Parties.CLIENT.id())));
  ...
  public void testTooManyParties() throws InterruptedException, ExecutionException {
    Map<Integer, NetworkConfiguration> conf = NetworkTestUtils.getNetworkConfigurations(3);
    ExecutorService es = Executors.newFixedThreadPool(3);
    Future<CloseableNetwork> n1 = es.submit(() -> new AsyncNetwork(conf.get(1)));
    es.submit(() -> new AsyncNetwork(conf.get(2)));
    es.submit(() -> new AsyncNetwork(conf.get(3)));
